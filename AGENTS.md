...
- ## Automated tests — directory layout, fixture rules, cleanup
    ...
    - **Hard rule — Test execution loop (build → background-run → notification → kill → read log):**
        ...
        3. **Launch the runner as a background command.** Use the harness's `run_in_background: true` (Claude Code) or your shell's job-control equivalent. Do NOT block the foreground waiting for completion.
        ...
        5. **Kill the dev app once the runner exits**, using exact-name matching (see the launching-the-app hard rule). The runner's own EXIT trap should already do this, but double-tap is safe and cheap.
        ...
        - The forbidden polling patterns above are forbidden even for "I just need a quick check" — they bias toward false-pass when the assertion you care about lands in the last frame of writes.
