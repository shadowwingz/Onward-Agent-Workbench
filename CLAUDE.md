...
- ## Automated tests — directory layout, fixture rules, cleanup
    ...
    - **Hard rule — Timing-sensitive autotest authoring (repeat-inside-the-test):**
        ...
        - Pick the aggregator by what you're measuring:
            - **Boolean correctness (recovers / doesn't recover)** → assert "all N trials succeeded" (or "≥ K of N", with K chosen against the failure cost). One miss across 5 trials means the recovery path has a real hole; absorb-and-retry hides it.
            - **Latency / throughput / pixel intensity** → assert on the median (or p95) of N trials, not the worst single sample. Drop top/bottom 10 % if the variance is bimodal (cold start vs warm path).
            - **State integrity (no listener / handle / cooldown accumulation)** → assert after the Nth trial, with a budget that does NOT grow with N (e.g. listener count must equal the baseline).
