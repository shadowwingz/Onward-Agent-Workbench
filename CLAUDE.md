...
- Hard rule — Automated / unit test iteration loop (run → exit → read → fix → rebuild → repeat):
    Applies whenever the user asks you to run tests, **or** whenever you are driving a fix-and-verify loop against unit tests, autotest runners, or `run-full-regression.py`. Every iteration follows this strict sequence in order — never overlap steps:
    1. **Run the test** as a single one-shot invocation. Foreground `Bash` is fine for short runs; for longer runs use `run_in_background: true` so the harness notifies you on exit. The test must write its log to a persisted file (autotest runners write `<repoRoot>/traces/test-logs/<suite>.log`; the regression orchestrator writes `test/full-regression-results/<timestamp>/`).
    2. **Wait for the test process to exit on its own.** For foreground runs this is automatic. For background runs, wait for the runtime's completion notification — do not poll, do not `sleep`-loop, do not `ScheduleWakeup` to peek early.
    3. **Exit / kill the application** if the test left an Electron build running (use the `pkill -x "<exact-process-name>"` pattern from the launch rule above). The next compile must start from a clean process table.
    4. **Read the persisted log file** with the `Read` tool after the process is gone.
    5. **Analyse the failure(s) and apply the code fix.**
    6. **Rebuild** via `rm -rf out release && pnpm dist:dev` (or the narrower build the change actually requires — e.g. `tsc --noEmit`, a unit-only re-run).
    7. **Re-run the test** as another one-shot, again wait for exit, again exit the app on completion, again read the log.
    Repeat 1–7 until green. Do not declare success based on partial output or a "looks fine so far" stream snapshot.
    **Forbidden** inside this loop: any "wait for a specific log line to appear" pattern. Do **not** use the `Monitor` tool to watch the test's log, do not `tail -f` into a polling loop, do not pipe the test's stdout into a `grep -m1` / `while read` waiter, do not `until grep -q … <log>; do sleep N; done`, and do not `Read` the log file repeatedly hoping a line shows up. The test owns its lifecycle; wait for the *process* to exit, then read the *file*. The persisted log is the source of truth, not a live stream.
    Rationale: live-stream waiters race with buffered output, mask early crashes (the line you're waiting for never gets flushed), entangle Claude's process tree with the test's, and waste cache window on idle polling. Run-then-analyse keeps the evidence chain reproducible and each iteration deterministic.
