...
LOG_FILE="${2:-/tmp/onward-terminal-architecture-baseline-autotest.log}"
RESULT_DIR="$ROOT_DIR/test/results/terminal-architecture-baseline"
mkdir -p "$RESULT_DIR"
...
node "$ROOT_DIR/test/prepare-terminal-architecture-baseline-fixture.mjs"

WORK_DIR="$ROOT_DIR/test/fixtures/terminal-architecture-baseline/workdir"
USER_DATA_DIR="$ROOT_DIR/test/fixtures/terminal-architecture-baseline/user-data"
case "$USER_DATA_DIR" in
  "$ROOT_DIR"/test/fixtures/terminal-architecture-baseline/user-data)
    rm -rf "$USER_DATA_DIR"
...
if [[ -n "$COMPARE_BASELINE" ]]; then
  ...
  node "$ROOT_DIR/test/compare-performance-baseline.mjs" \
