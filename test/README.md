...
| Prompt editor right-click context menu — undo / cut / copy / paste / clear-content / pinned-import / save-as-pinned / insert cwd / insert branch / insert task title / send-to-task; auto viewport flip + clamp. Also locks down the textarea's virtual-cursor behaviour: click-anywhere padding to (row, col), IME guard, paste at virtual position, undo of virtual padding, submit-time stripping of trailing whitespace / empty rows; AND the per-Tab Canvas/Line input-mode dropdown in the title row (default Canvas, Line disables virtual click, Canvas restores it, Line still submits). | `run-prompt-editor-context-menu` (PECM-01..03, 05..09, 13..27) |
| Send-transform pure function — strips per-line trailing whitespace and trailing empty rows so virtual-cursor placements with no input do not leak to the terminal. | `node --test test/unittest/prompt-virtual-padding.test.mts` (PVP-U-01..08, 10..13). Out-of-band unit; not in `SCRIPTS`. |
