...
# ---------------------------------------------------------------------------
# Build the fixture under test/fixtures/git-diff-staleness-and-submodule/runtime/
...
# ---------------------------------------------------------------------------
FIXTURE_JSON="$(node "$REPO_ROOT/test/create-git-diff-staleness-fixture.mjs")"
...
#
# We delegate matching to test/check-trace-event.mjs so the parser handles
...
expect_event() {
  ...
  local needle="$2"
  local match
  if match="$(node "$REPO_ROOT/test/check-trace-event.mjs" \
    --main "$TRACE_FILE" \
