...
$ResultDir = Join-Path $RootDir "test\results\prompt-input-latency"
New-Item -ItemType Directory -Force $ResultDir | Out-Null

if (-not $LogFile) {
  $LogFile = Join-Path $env:TEMP "onward-prompt-input-latency-autotest.log"
}
...
$prepareScript = Join-Path $RootDir "test\prepare-prompt-input-latency-fixture.mjs"
$fixtureSummary = node $prepareScript | Out-String
Write-Host $fixtureSummary

$WorkDir = Join-Path $RootDir "test\fixtures\prompt-input-latency\workdir"
$UserDataDir = Join-Path $RootDir "test\fixtures\prompt-input-latency\user-data"
