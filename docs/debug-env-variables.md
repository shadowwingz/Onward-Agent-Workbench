...
| `ONWARD_PERF_TRACE_CAPTURE_CONTENT` | `1` | Include bounded raw Prompt / terminal / render content previews in the local performance trace (codex `performance-trace` writer). Requires `ONWARD_PERF_TRACE=1`; sensitive content is omitted by default. |
